...
class PromptManager:
    ...
    def build_system_prompt(
        ...
        将角色 Prompt、系统规则 Prompt、记忆上下文组合在一起。

        Args:
            character_prompt: 角色的系统 Prompt（由 Character.to_system_prompt() 生成）
            memory_context: 记忆上下文（由 MemoryManager.get_memory_context() 生成）
        ...
        # 1. 角色 Prompt（放在最前面，让 LLM 优先理解角色设定）
        if character_prompt:
            parts.append(character_prompt)

        # 2. 系统规则 Prompt
        now = datetime.now().strftime("%Y年%m月%d日 %H:%M")
        system_rules = self.render("system", current_time=now)
        if system_rules:
